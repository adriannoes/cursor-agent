...
- [ ] **1.0 Configurar projeto e dependências (`pyproject.toml`)** — PRD T1 (T1.1–T1.3)
  ...
  **Acceptance criteria:**
  ...
  - [ ] 1.1 Fixar `cursor-sdk` com pin exato no `pyproject.toml`
  ...
  - [ ] 1.2 Adicionar dependências de desenvolvimento (`pytest`, `pytest-asyncio`, `pytest-cov`, `ruff`, `mypy`)
  ...
  - [ ] 1.3 Configurar `[tool.pytest.ini_options]` com marker `integration` e `asyncio_mode`
    - **File**: `pyproject.toml` (modify existing)
  ...
  - [ ] 1.4 Gerar lockfile e validar ambiente com `uv sync`
  ...
  - [ ] 1.5 Validar coleta pytest sem integração
  ...
  - [ ] 1.6 Configurar `[tool.ruff]`, `[tool.mypy]` e `[tool.coverage.run]` no `pyproject.toml`
    - **File**: `pyproject.toml` (modify existing)
  ...
  - [ ] 1.7 Teste TDD `tests/test_pyproject_config.py` (markers + `asyncio_mode`)
  ...
  - [ ] 1.8 Adicionar workflow CI mínimo `.github/workflows/ci.yml`
...
- [ ] **2.0 Implementar exemplo async REPL (`examples/async_repl.py`)** — PRD T2 (T2.1–T2.3)
  ...
  **Acceptance criteria:**
  ...
  - [ ] 2.1 Criar esqueleto do exemplo com `AsyncClient.launch_bridge`
  ...
  - [ ] 2.2 Fixar modelo Composer 2.5 e documentar cold start medido
  ...
  - [ ] 2.3 Implementar segundo turn com contexto preservado
    - **File**: `examples/async_repl.py` (modify existing)
  ...
  - [ ] 2.4 Registrar decisão local-first em comentário inline
    - **File**: `examples/async_repl.py` (modify existing)
  ...
  - [ ] 2.5 Validar pré-condição `CURSOR_API_KEY` com mensagem acionável
    - **File**: `examples/async_repl.py` (modify existing)
...
- [ ] **3.0 Implementar inventário de tools e gerar snapshot** — PRD T3 + T5
  ...
  **Acceptance criteria:**
  ...
  - [ ] 3.1 Descobrir e documentar a API de introspecção de tools do SDK
  ...
  - [ ] 3.2 Persistir inventário em `docs/sdk-tools-snapshot.txt`
  ...
  - [ ] 3.3 Documentar procedimento de regeneração do snapshot
...
- [ ] **4.0 Implementar smoke test de integração (`tests/integration/test_sdk_smoke.py`)** — PRD T4 (T4.1–T4.2) + FR-6
  ...
  **Acceptance criteria:**
  ...
  - [ ] 4.1 Criar pacote `tests/integration/` e arquivo de smoke test
  ...
  - [ ] 4.2 Aplicar marker `@pytest.mark.integration` em todos os testes do módulo
  ...
  - [ ] 4.3 Implementar skip automático quando `CURSOR_API_KEY` estiver ausente
    - **File**: `tests/integration/test_sdk_smoke.py` (modify existing)
  ...
  - [ ] 4.4 Testar inicialização do bridge e turn básico async
    - **File**: `tests/integration/test_sdk_smoke.py` (modify existing)
  ...
  - [ ] 4.5 Testar turn que aciona tool nativa (grep ou leitura de arquivo)
    - **File**: `tests/integration/test_sdk_smoke.py` (modify existing)
...
- [ ] **5.0 Validar Definition of Done e handoff para Fase 1**
  ...
  **Acceptance criteria:**
  ...
  - [ ] 5.1 Executar gate de qualidade local (sem integração)
  ...
  - [ ] 5.2 Executar **`/code-review`** antes do handoff
  ...
  - [ ] 5.3 Executar smoke de integração com API key válida
  ...
  - [ ] 5.4 Validar Definition of Done e métricas do PRD §8
  ...
  - [ ] 5.5 Confirmar ausência de artefatos da Fase 1 e preparar handoff
  ...
  - [ ] 5.6 Revisar PRD-001 com aprendizados do spike (retroalimentação)

...
5.  **Phase 2: Generate Sub-Tasks:** Once the user confirms, break down each parent task into smaller, actionable sub-tasks. **Use the detailed format** (see below) to ensure weaker AI models can execute correctly. For tasks that are part of a flow or have dependencies, add **Trigger/entry point**, **Enables**, and **Depends on** at the task level. Give each parent task its own **acceptance criteria** (verifiable, and specific to that task).
    *   **TDD ordering ([ADR-022](../../docs/decisions/ADR-022-tdd-prd-feedback-loop.md)):** Sub-tasks that implement a functional requirement must list **Verify** with a **pytest command first** (red → green). The production code sub-task follows the failing-test sub-task for the same FR.
    *   **PRD retrospective:** The final parent task (typically **5.0**) must include a sub-task to update the **next PRD** in the retro chain (see PRD §11) with learnings before starting that PRD — e.g. "Revisar PRD-001 com aprendizados do spike".
...
- [ ] X.Y.Z [Action verb] [specific item]
  ...
  - **Verify**: [How to confirm it works - **pytest command first** (TDD red→green), e.g. `pytest tests/... -k "test_fr3" -v` must fail before implementation, then pass]
```
...
- **PRD Template**: [prd.md](./prd.md) - How to create PRDs
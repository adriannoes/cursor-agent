...
The gateway inherits Memory v1 through the shared send path. If `~/.cursor-agent/USER.md` or `~/.cursor-agent/MEMORY.md` exist, the first free-text message after `/new` receives the same bounded memory payload as the CLI. Telegram does not expose `/memory show`; use the CLI command when you need to inspect the effective payload.

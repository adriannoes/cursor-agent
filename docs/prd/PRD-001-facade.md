...
- [x] Layout `src/cursor_agent/sdk_facade.py` + pacote `tests/unit/` criado (T0)
- [x] Handoff PRD-000 preservado (`__version__`, `test_package_metadata.py`, gate CI local sem key)
- [x] `src/` não importa `examples/` (spike isolado da API de produção)
- [x] Contrato implementado conforme [contracts/async-sdk-facade.md](../contracts/async-sdk-facade.md)
- [x] `FakeSdkFacade` + testes unitários
- [x] Logs JSON v1 em send start/end
...
- [x] T0.1 `tests/unit/__init__.py` — pacote pytest para testes da facade
- [x] T0.2 `src/cursor_agent/sdk_facade.py` — stub inicial (tipos + `SdkFacade` Protocol **sem** import `cursor_sdk` até T2)
- [x] T0.3 `src/cursor_agent/__init__.py` — preservar `__version__`; re-export opcional de tipos públicos (`RunResult`, `SdkFacade`, …)
- [x] T0.4 `tests/unit/test_facade_imports.py` — RED→GREEN: apenas `sdk_facade` importa `cursor_sdk` quando `AsyncSdkFacade` existir
- [x] T0.5 Gate ADR-026 verde após scaffold (`test_package_metadata.py` incluído; cobertura ≥85%)
...
- [x] T2.1 `launch_bridge` context manager
- [x] T2.2 create / resume / send / cancel
- [x] T2.3 MCP re-inject no resume
- [x] T2.4 retry `is_retryable`
- [x] T2.5 StreamCallbacks
...
- [x] Forma real de `RunResult`, `StreamCallbacks` e erros do SDK vs. contrato — `run.wait()` no SDK 0.1.7 retorna `RunResult` com `id` (não `run_id`), `status` (`finished`/`error`/`cancelled`), `result` (texto) e `duration_ms` (int); não há campo `usage` — o adapter deriva `usage={"duration_ms": N}` quando `duration_ms` está presente. Stream único: `async for message in run.messages()` + `await run.wait()`; callbacks em mensagens `tool_call` (`status` `running`/`completed`/`error`). Erros `cursor_sdk.errors` mapeados por `isinstance` para taxonomia ADR-024 (`AuthenticationError`→`AuthError`, `RateLimitError`/`InternalServerError`→`NetworkError`, `APITimeoutError`→`TimeoutError`, `AgentNotFoundError`→`InvalidAgentError`); fallback duck-typing para exceções Python (`ConnectionError`→`NetworkError`). SDK expõe `retry_after` como `str | None` — facade parseia para `float` antes do backoff.
- [x] Latência de create/resume/send com bridge real (se smoke cruzado) — cold start do bridge local permanece na ordem de ~4s+ por turn (mesmo spike PRD-000); `tests/integration/test_facade_shape.py` reutiliza padrão do smoke com `AsyncSdkFacade` e skip sem `CURSOR_API_KEY`; gate de PR continua sem integração.
- [x] Comportamento de MCP re-inject no resume (confirmado ou ajustar contrato) — `_resolve_mcp_servers` stub retorna `{}` para perfis `coding` e `messaging`; `resume_agent` passa `mcp_servers` nas options do SDK (inline não persiste). Validado unitariamente com mock de `client.agents.resume`; perfil real adiado ao PRD-005.
- [x] Campos de log que o SDK expõe vs. schema v1 — SDK não emite NDJSON; facade emite `send_start`/`send_end` via `facade_logging.py` com schema v1 (`v`, `ts`, `level`, `event`, `agent_id`, `run_id`, `duration_ms`, `status`). `session_id`/`session_key` opcionais (`null`) até PRD-002; redaction ADR-025 em strings de `agent_id`.
- [x] Ajustes em estimativas de pool/lock (PRD-002) com base no fake — `FakeSdkFacade.send_in_progress` (`asyncio.Event`) pronto para simular run longo em `SessionAgentPool.try_acquire`; fake **não** levanta `AgentBusyError` (ADR-008). Estimativas T1/T2 (6h cada) mantidas — fake desbloqueia TDD de pool sem bridge real.

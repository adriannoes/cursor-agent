...
| Cron runtime | Use **`local` only** for new jobs in **v1.3.1**. The CLI still accepts `--runtime cloud` until **v1.3.2**, but execution uses local SDK options — not an isolated VM. **v1.3.2** will reject cloud before side effects |
